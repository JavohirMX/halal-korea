...
def monitoring_dashboard(request):
    ...
    context = {
        ...
        "data_management_url": "/admin/places/data-management/",
    }
        ...
        if log_path.exists():
            ...
            try:
                ...
                with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    if level:
                        last_lines = [
                            line for line in last_lines if level.upper() in line.upper()
                        ]
    ...
    if log_path.exists():
        ...
        try:
            with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                if search:
                    last_lines = [
                        line for line in last_lines if search.lower() in line.lower()
                    ]
...
def api_metrics(request):
    ...
    metrics = (
        SystemMetric.objects.filter(metric_type=metric_type, timestamp__gte=start_time)
        .order_by("timestamp")
...
def api_stats(request):
    ...
    stats = {
        ...
        "errors": RequestLog.objects.filter(
            timestamp__gte=start_time, status_code__gte=500
        ).count(),
...
def api_performance(request):
    ...
    data = {"endpoints": list(slow_endpoints)}
    ...
    return JsonResponse(
        {
            ...
            "geographic_distribution": ga_data.get(
...
def api_chart_data(request):
    ...
    hourly_stats = (
        ...
        .values("hour")
        .annotate(count=Count("id"), errors=Count("id", filter=Q(status_code__gte=500)))
        .order_by("hour")
    )

    # Error distribution
    error_dist = (
        RequestLog.objects.filter(timestamp__gte=last_24h, status_code__gte=400)
        .values("status_code")
...
def _get_today_stats():
    ...
    error_count = RequestLog.objects.filter(
        timestamp__gte=today_start, status_code__gte=500
    ).count()

    avg_response = RequestLog.objects.filter(timestamp__gte=today_start).aggregate(
        avg=Avg("response_time_ms")
    )

    active_users_count = User.objects.filter(last_login__gte=today_start).count()
...
def _get_recent_errors():
    ...
    return RequestLog.objects.filter(status_code__gte=500).order_by("-timestamp")[:10]


def _get_recent_security_events():
    """Get recent security events."""
    return SecurityEvent.objects.filter(resolved=False).order_by("-timestamp")[:10]
...
def _get_response_time_stats(days):
    ...
    return RequestLog.objects.filter(timestamp__gte=start_time).aggregate(
        avg=Avg("response_time_ms"),
...
def _get_db_query_stats(days):
    ...
    return RequestLog.objects.filter(timestamp__gte=start_time).aggregate(
        avg_queries=Avg("db_query_count"), max_queries=Max("db_query_count")
    )
...
def _get_cache_stats(days):
    ...
    cache_data = RequestLog.objects.filter(timestamp__gte=start_time).aggregate(
        total_hits=Sum("cache_hits"), total_misses=Sum("cache_misses")
    )
...
def _get_failed_logins():
    ...
    return SecurityEvent.objects.filter(event_type="failed_login").order_by(
        "-timestamp"
    )[:20]
...
def _get_rate_limit_hits():
    ...
    return SecurityEvent.objects.filter(event_type="rate_limit_hit").order_by(
        "-timestamp"
    )[:20]
...
def _get_suspicious_activity():
    ...
    return SecurityEvent.objects.filter(
        event_type="suspicious_activity", resolved=False
...
def _get_moderator_activity():
    ...
    activity = (
        ContentModerationLog.objects.filter(timestamp__gte=last_7_days)
...
def _get_submission_trends():
    ...
    places_by_day = (
        HalalPlace.objects.filter(created_at__gte=last_30_days)
...
def _get_quality_metrics():
    ...
    return {
        "approval_rate": (approved_places / total_places * 100)
        if total_places > 0
        else 0,
        "rejection_rate": (rejected_places / total_places * 100)
        if total_places > 0
        else 0,
        "places_with_images": places_with_images,
        "image_coverage": (places_with_images / total_places * 100)
        if total_places > 0
        else 0,
    }
...
def _get_content_usage(days):
    ...
    most_viewed_places = HalalPlace.objects.filter(status="approved").order_by("-id")[
        :10
    ]  # Would need view_count field

    most_reviewed = HalalPlace.objects.annotate(review_count=Count("reviews")).order_by(
        "-review_count"
    )[:10]
    ...
    result = {
        ...
        "popular_categories": list(
            HalalPlace.objects.values("category")
            .annotate(count=Count("id"))
            .order_by("-count")
        ),
    }
...
def _get_yesterday_stats():
    ...
    total_requests = RequestLog.objects.filter(
        timestamp__gte=yesterday_start, timestamp__lt=yesterday_end
    ).count()
...
def _get_health_threshold(condition):
    ...
    defaults = {
        "health_error_rate_warning": getattr(
            settings, "MONITORING_HEALTH_ERROR_RATE_WARNING", 5.0
        ),
        "health_error_rate_critical": getattr(
            settings, "MONITORING_HEALTH_ERROR_RATE_CRITICAL", 10.0
        ),
        "health_response_time_warning": getattr(
            settings, "MONITORING_HEALTH_RESPONSE_TIME_WARNING", 2000
        ),
...
def _get_system_health():
    ...
    recent_errors = RequestLog.objects.filter(
        timestamp__gte=last_hour, status_code__gte=500
    ).count()
    error_rate = (recent_errors / recent_requests * 100) if recent_requests > 0 else 0

    # Check response time
    avg_response = (
        RequestLog.objects.filter(timestamp__gte=last_hour).aggregate(
            avg=Avg("response_time_ms")
        )["avg"]
        or 0
    )
    ...
    if (
        not db_healthy
        or not cache_healthy
        or critical_events > 0
        or error_rate > error_rate_critical
    ):
        status = "critical"
...
def _get_unread_notification_count(user):
    ...
    return AdminNotification.objects.filter(
        Q(recipient=user) | Q(recipient__isnull=True), read=False, dismissed=False
    ).count()
...
def _get_error_rate_trend(days):
    ...
    daily_stats = (
        ...
        .values("date")
        .annotate(total=Count("id"), errors=Count("id", filter=Q(status_code__gte=500)))
        .order_by("date")
    )
...
def _get_response_time_trend(days):
    ...
    daily_stats = (
        ...
        .annotate(date=TruncDate("timestamp"))
        .values("date")
        .annotate(avg_time=Avg("response_time_ms"), max_time=Max("response_time_ms"))
        .order_by("date")
    )
